...
public class AccessToken {
    public AccessToken(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        HttpSession session = request.getSession();
        String login_id = (String) session.getAttribute("login_id");
        String password = (String) session.getAttribute("password");
        String url = "https://qa2.sunbasedata.com/sunbase/portal/api/assignment_auth.jsp"; // Update the URL
        String jsonInput = "{\n" +
                "\"login_id\" : \"" + login_id + "\",\n" +
                "\"password\" :\"" + password + "\"\n" +
                "}";
